...
```
#### 5. Seed the database
If the migration is successfull, seed the database with roles and permissions and add default users. This will create two users; `admin@admin.com` and `timetabler@mail.com`, `teacher@teacher.com` with `password` as their passwords respectively.
```bash
php artisan db:seed
```
